...
static jpeg_error_t esp_jpeg_decoder_one_image(uint8_t *input_buf, int len,
                                               uint8_t *output_buf) {
...
void show_new_pic() {
    if (v_fileContent.size() > 0) {
        ...
        while (loadFile.available() && bytesRead < MAX_PIC_FILE_SIZE) {
            size_t len = min((size_t)loadFile.available(),
                             min(sizeof(buff), MAX_PIC_FILE_SIZE - bytesRead));
        ...
        ret = esp_jpeg_decoder_one_image(img_read_data, bytesRead,
                                         (uint8_t *)pic_img_data);
        if (ret != JPEG_ERR_OK) {
...
#define MJPEG_BUFFER_SIZE \
    (MJPEG_OUTPUT_SIZE / 20)  // memory for a single JPEG frame
...
void show_new_mjpeg() {
    if (v_fileContent.size() < 1) return;
    ...
    if (!mjpegFile || mjpegFile.isDirectory()) {
        ...
        if (!mjpeg.setup(&mjpegFile, mjpeg_buf, pic_img_data, MJPEG_OUTPUT_SIZE,
                         true /* useBigEndian */)) {
...
void setup() {
    ...
    bsp_display_unlock();
#elif CURR_DEMO == DEMO_PIC
    pic_img_data = (uint16_t *)heap_caps_aligned_alloc(16, 480 * 320 * 2,
                                                       MALLOC_CAP_SPIRAM);
    memset(pic_img_data, 0, 480 * 320 * 2);
    img_read_data =
        (uint8_t *)heap_caps_malloc(MAX_PIC_FILE_SIZE, MALLOC_CAP_SPIRAM);
    memset(img_read_data, 0, MAX_PIC_FILE_SIZE);
    ...
    pic_img_data = (uint16_t *)heap_caps_aligned_alloc(16, MJPEG_OUTPUT_SIZE,
                                                       MALLOC_CAP_SPIRAM);
