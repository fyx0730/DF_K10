...
lv_obj_t * add_list_btn(lv_obj_t * parent, uint32_t List_id)
{
  ...
  lv_label_set_text(title_label, Song_Name[List_id]);                       
...
void LVGL_Search_Music() {        
  ...
  if(ACTIVE_TRACK_CNT) {  
    for (int i = 0; i < ACTIVE_TRACK_CNT; i++) {
      strcpy(Song_Name[i], File_Name[i]);
      remove_file_extension(Song_Name[i]); 
    }                
    LVGL_Play_Music(0);    
  }                                                             
}
void LVGL_Play_Music(uint32_t ID) {
  Play_Music("/",File_Name[ID]);
  // printf(File_Name[ID]);
  strncpy(Audio_Name,Song_Name[ID], sizeof(Song_Name[ID]));       
  // strncpy(Audio_Name,Song_Name[ID], sizeof(Audio_Name));       
  Audio_duration = Music_Duration();  
